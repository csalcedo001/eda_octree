...
ScanOctree::ScanOctree(int width, int height, int depth, double threshold) :
	BaseImageOctree(width, height, depth, threshold)
{ }

ScanOctree::ScanOctree(Image3D &image, double threshold) :
	BaseImageOctree(image, threshold)
{
	this->build(this->head_, image, 0, this->width_ - 1, 0, this->height_ - 1, 0, this->depth_ - 1);
}
...
void ScanOctree::build(Node<Pixel> *&node, Image3D &image, int x_i, int x_f, int y_i, int y_f, int z_i, int z_f) {
	int mid_x = (x_i + x_f) / 2;
	int mid_y = (y_i + y_f) / 2;
	int mid_z = (z_i + z_f) / 2;
	...
	if (this->same_color(image, x_i, x_f, y_i, y_f, z_i, z_f)) {
	...
	this->build(node->children_[0], image, x_i, mid_x, y_i, mid_y, z_i, mid_z);
	this->build(node->children_[1], image, mid_x + 1, x_f, y_i, mid_y, z_i, mid_z);
	this->build(node->children_[2], image, x_i, mid_x, mid_y + 1, y_f, z_i, mid_z);
	this->build(node->children_[3], image, mid_x + 1, x_f, mid_y + 1, y_f, z_i, mid_z);
	this->build(node->children_[4], image, x_i, mid_x, y_i, mid_y, mid_z + 1, z_f);
	this->build(node->children_[5], image, mid_x + 1, x_f, y_i, mid_y, mid_z + 1, z_f);
	this->build(node->children_[6], image, x_i, mid_x, mid_y + 1, y_f, mid_z + 1, z_f);
	this->build(node->children_[7], image, mid_x + 1, x_f, mid_y + 1, y_f, mid_z + 1, z_f);
}

bool ScanOctree::same_color(Image3D &image, int x_i, int x_f, int y_i, int y_f, int z_i, int z_f) {
